...
while True:
    ...
    img = cv2.flip(img,-1)# Flip frame vertically, webcam was positioned upside down 
    ...
    thresh, gray_img = cv2.threshold(gray_img, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    ...
    abs_img = cv2.erode(abs_img,erode_kernel,iterations= 4)

    #To provide boundary for connected blobs and draw bounding rect of those boundary on the original frame 
    (contours,_) = cv2.findContours(abs_img,cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    for contour in contours:
        if cv2.contourArea(contour) > 500: # If blobs area is more than x value, means movement/foreign object detected
    ...
    if record:
        ...
        record_end_time = time.time() + 30 #run for 30 seconds
        print("Recording...")
        ...
        base_img = [] #to reset base image (To prevent device from constantly detecting unexpected noises)
